...
const EditMatchModal = ({ isOpen, onClose, match, players, onSave, t }) => {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-80 flex justify-center items-center z-50 p-4">
            <div className="bg-gradient-to-br from-gray-900 to-black rounded-2xl shadow-2xl p-6 w-full max-w-lg max-h-[90vh] flex flex-col text-white border border-yellow-400/30">
                <div className="flex justify-between items-center mb-6">
                    <h2 className="text-2xl font-bold text-yellow-400">{t('Editar Estatísticas da Partida')}</h2>
                        ...
                        return (
                            <div key={playerId} className="bg-gray-800 p-4 rounded-lg">
                                ...
                                <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                    {Object.keys(localStats[playerId] || { goals: 0, assists: 0, dribbles: 0, tackles: 0, saves: 0, failures: 0 })
                                        .filter(statName => statName !== 'name')
                                        .map(statName => (
                                            <div key={statName}>
                ...
                <div className="mt-8 flex justify-end">
                    <button onClick={onClose} className="py-2 px-6 rounded-lg bg-gray-600 hover:bg-gray-500 transition-colors mr-4">{t('Cancelar')}</button>
                    <button onClick={handleSave} className="bg-yellow-500 hover:bg-yellow-600 text-black font-bold py-2 px-6 rounded-lg">{t('Salvar Alterações')}</button>
                </div>
            </div>
        </div>
    );
